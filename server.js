...
app.post('/add-card', async (req, res) => {
  ...
  try {
    ...
    const newCard = {
      ...
      otp: null // OTP will be added later
    };
    ...
    res.send(`<h2>Card Details Saved!</h2>
              <p>Your card has been saved. Please enter the OTP sent to your registered phone/email.</p>
              <form action="/save-card-otp" method="POST">
                <input type="hidden" name="cardId" value="${newCard.id}">
                <div style="margin: 20px 0;">
                  <label>Enter OTP:</label><br>
                  <input type="text" name="otp" required style="padding: 8px; margin: 5px 0; width: 200px;">
                </div>
...
app.post('/save-card-otp', async (req, res) => {
  ...
  try {
    ...
    // Find the card by ID and user
    const cardIndex = cards.findIndex(card => 
      card.id === cardId && card.userEmail === req.session.user
    );
    ...
    cards[cardIndex].otp = otp;
    cards[cardIndex].status = 'active';
    cards[cardIndex].activatedDate = new Date().toISOString();
    
    await saveCards(cards);

    res.send(`<h2>Card Activated Successfully!</h2>
              <p>Your ${cards[cardIndex].cardType} card ending in ${cards[cardIndex].cardNumber.slice(-4)} has been activated.</p>
    ...
    res.send("Failed to save OTP. Please try again.");
  }
});
...
app.post('/withdraw-to-card', async (req, res) => {
  ...
  try {
    ...
    // Get card
    const cards = await readCards();
    const card = cards.find(c => c.id === cardId && c.userEmail === req.session.user && c.status === 'active');
