...
app.post('/add-card', async (req, res) => {
    ...
    try {
        const { cardNumber, cardHolder, expiryDate, cvv, otp } = req.body;
        
        // Basic validation
        if (!cardNumber || !cardHolder || !expiryDate || !cvv || !otp) {
            return res.send("All fields are required.");
        }
        
        // Validate card number (16 digits)
        if (!/^\d{16}$/.test(cardNumber.replace(/\s/g, ''))) {
            return res.send("Invalid card number. Must be 16 digits.");
        }
        
        // Validate expiry date (MM/YY format)
        if (!/^\d{2}\/\d{2}$/.test(expiryDate)) {
            return res.send("Invalid expiry date. Use MM/YY format.");
        }
        
        // Validate CVV (3 digits)
        if (!/^\d{3}$/.test(cvv)) {
            return res.send("Invalid CVV. Must be 3 digits.");
        }
        
        // Validate OTP (6 digits)
        if (!/^\d{6}$/.test(otp)) {
            return res.send("Invalid OTP. Must be 6 digits.");
        }
        ...
        const cardExists = user.cards.some(card => card.cardNumber === cardNumber.replace(/\s/g, ''));
        if (cardExists) {
            return res.send("This card is already added to your account.");
        }
        
        // Create new card object
        const newCard = {
            id: Date.now().toString(), // Unique ID for the card
            ...
            isDefault: user.cards.length === 0 // First card becomes default
        };
        
        // Add card to user's cards
        user.cards.push(newCard);
        
        // Log the card addition as a transaction
        user.transactions.push({
            type: "Card Added",
            description: `New card ending with ${cardNumber.slice(-4)} added`,
            date: new Date().toISOString(),
            balanceAfterTransaction: user.balance
        });
        
        await saveUsers(users);
        await logTransaction(user.email, 'Card Added', 0);
        
        res.send(`<h2>Card Added Successfully!</h2>
            <p>Your card ending with ${cardNumber.slice(-4)} has been added to your account.</p>
            <p>Card Holder: ${cardHolder.toUpperCase()}</p>
            <p>Expiry Date: ${expiryDate}</p>
            <p>Redirecting to cards page...</p>
            <script>
                setTimeout(() => window.location.href = '/card.html', 3000);
            </script>`);
    } catch (error) {
        console.error('Add card error:', error);
        res.send("Failed to add card. Please try again.");
    }
});
...
app.post('/set-default-card', async (req, res) => {
    ...
    try {
        ...
        if (!user || !user.cards) {
            return res.status(404).json({ error: "User or cards not found" });
        }
        ...
        user.cards[cardIndex].isDefault = true;
...
app.post('/delete-card', async (req, res) => {
    ...
    try {
        ...
        user.transactions.push({
            type: "Card Removed",
            description: `Card ending with ${cardToDelete.cardNumber.slice(-4)} removed`,
            date: new Date().toISOString(),
            balanceAfterTransaction: user.balance
        });
        
        await saveUsers(users);
        await logTransaction(user.email, 'Card Removed', 0);
...
app.get('/check-session', (req, res) => {
    if (req.session.user) {
        res.json({ loggedIn: true, user: req.session.user, userName: req.session.userName });
    } else {
...
async function initializeServer() {
    ...
    if (connectionSuccess) {
        ...
        console.log('  - POST /set-default-card');
