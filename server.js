...
app.post('/api/chat/send', async (req, res) => {
  ...
  try {
    ...
    // Send ntfy notification for messages involving admin
    const isAdminInvolved = (from === admin.email || from === admin.phone || to === admin.email || to === admin.phone);
...
app.get('/api/chat/unread', async (req, res) => {
  ...
  try {
    ...
    chats.forEach(conversation => {
      if (conversation.participants && conversation.participants.includes(currentUser)) {
  ...
  try {
    ...
    if (!req.session.isAdmin) {
      ...
      try {
        ...
        if (conversation) {
          adminUser.unreadCount = conversation.messages.filter(m => m.to === currentUser && !m.read).length;
          
          // Get last message
          const lastMsg = conversation.messages[conversation.messages.length - 1];
          if (lastMsg) {
            adminUser.lastMessage = lastMsg.message.substring(0, 30) + (lastMsg.message.length > 30 ? '...' : '');
            adminUser.lastMessageTime = lastMsg.timestamp;
          }
        }
      } catch (e) {
...
// Mark messages as read
app.post('/api/chat/mark-read/:otherUser', async (req, res) => {
  if (!req.session.user) {
    return res.status(401).json({ error: "Not logged in" });
  }
...
async function initializeServer() {
  ...
  if (connectionSuccess) {
    ...
    console.log('  - POST /api/chat/mark-read/:otherUser');
    console.log('\n=== ADMIN ENDPOINTS ===');
