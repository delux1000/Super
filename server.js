...
const CARDS_BIN_ID = '6937224e043b5e708f8c086e'; // New bin ID for cards
...
async function readJSONBin(url) {
  try {
    ...
    if (!response.ok) {
      // If 404 (bin not found), return empty array
...
async function testJSONBinConnection() {
  try {
    ...
    // Test cards bin - create if it doesn't exist
...
app.post('/register', async (req, res) => {
  try {
    ...
    const newUser = {
      ...
      cards: [], // Initialize empty cards array for user
      transactions: [
        {
          type: 'Credit',
...
app.post('/add-card', async (req, res) => {
  ...
  try {
    ...
    if (cleanCardNumber.length < 15) { // Minimum 15 for Amex, 16 for others
