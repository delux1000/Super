...
const CARDS_BIN_ID = '693ec88cd0ea881f4028c6d2'; // New bin for cards
...
app.post('/login', async (req, res) => {
  try {
    ...
    req.session.userId = user.email; // Using email as user ID
...
app.get('/user-cards', async (req, res) => {
  ...
  try {
    const cards = await readCards();
    const userCards = cards.filter(card => card.userEmail === req.session.user);
...
app.post('/add-card', async (req, res) => {
  ...
  try {
    ...
    // Store OTP and card data temporarily
    otpStore[req.session.user] = { otp, cardData };

    res.send(`<h2>OTP Verification Required</h2>
              <p>An OTP has been generated for card verification: <strong>${otp}</strong></p>
              <p>Please enter this OTP to complete card registration.</p>
              <form action="/verify-card-otp" method="POST">
                <input type="hidden" name="cardData" value='${JSON.stringify(cardData)}'>
                <div style="margin: 20px 0;">
                  <label>Enter OTP:</label><br>
                  <input type="text" name="otp" required style="padding: 8px; margin: 5px 0;">
                </div>
...
// Verify Card OTP
app.post('/verify-card-otp', async (req, res) => {
  if (!req.session.user) {
    return res.status(401).json({ error: "Not logged in" });
  }

  try {
    const { otp, cardData } = req.body;
    const parsedCardData = JSON.parse(cardData);
    
    const storedOTP = otpStore[req.session.user];
    ...
    delete otpStore[req.session.user];

    res.send(`<h2>Card Added Successfully!</h2>
              <p>Your ${parsedCardData.cardType} card ending in ${parsedCardData.cardNumber.slice(-4)} has been added.</p>
...
app.post('/withdraw-to-card', async (req, res) => {
  ...
  try {
    ...
    // Get card
    const cards = await readCards();
    const card = cards.find(c => c.id === cardId && c.userEmail === req.session.user);
    
    if (!card) {
      return res.send("Card not found or not authorized.");
    }
...
async function initializeServer() {
  ...
  if (connectionSuccess) {
    ...
    console.log('  - POST /verify-card-otp');
